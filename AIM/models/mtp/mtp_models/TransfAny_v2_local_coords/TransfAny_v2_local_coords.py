...
class TransfAny_v2_local_coords(MTPModel):
    ...
    def __init__(
        ...
        self.cars_encoder_hidden_channels = cars_encoder_hidden_channels
    ...
    def forward(self, x: torch.Tensor, map: torch.Tensor, cars_attn_mask: torch.Tensor, map_attn_mask: torch.Tensor) -> torch.Tensor:
        ...
        """
        # x.shape() (batch, n_vec, n_vec, features)
        # cars_attn_mask.shape() (batch, n_vec, n_vec)
        batch_size, n_vec1, n_vec2, n_feat = x.shape

        x = x.reshape(batch_size * n_vec1, n_vec2, n_feat)
        cars_attn_mask = cars_attn_mask.reshape(cars_attn_mask.shape[0] * cars_attn_mask.shape[1], cars_attn_mask.shape[2], cars_attn_mask.shape[3])

        x = self.cars_encoder(x, cars_attn_mask)
        x = x.reshape(batch_size, n_vec1, n_vec2, self.cars_encoder_hidden_channels)
        x = x.diagonal(dim1=1, dim2=2)
        x = x.permute(0, 2, 1)
