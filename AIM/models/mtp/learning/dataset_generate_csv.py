...
if __name__ == "__main__":
    ...
    nets_dir_path = os.path.join(path_config.paths.data_path, nets_dir)

    if os.path.exists(path_config.paths.sumo_gened_maps_path):  # delete directory with old data if exists
    ...
    for net_file_name in os.listdir(nets_dir_path):
        ...
        sumo_files_path = os.path.join(path_config.paths.sumo_gened_maps_path, net_name)
        ...
        shutil.copytree(os.path.join(path_config.paths.data_path, "base_sumocfg"), os.path.join(sumo_files_path, "sumocfg"))
        ...
        generate_routefile(
            ...
            max_vehicle_accel=config.vehicle.accel * map_bounding,
            vehicle_decel=config.vehicle.decel * map_bounding,
            vehicle_sigma=config.vehicle.sigma,
            vehicle_length=config.vehicle.length * map_bounding,
            vehicle_max_speed=config.vehicle.max_speed * map_bounding,
            vehicle_minGap=config.vehicle.min_gap * map_bounding,
        )
        sumocfg_path = generate_sumocfg(sumo_files_path, route_filename, net_file_name)

        fcd_dir_path = os.path.join(path_config.paths.sumo_gened_maps_path, net_name, "fcd")
        csv_dir_path = os.path.join(path_config.paths.data_path, "csv", split, net_name)
        ...
        generate_fcd(sumocfg_path, fcd_file, 0, 0, num_seconds, config.temporal.dt, TRAFFIC_SCALE)
