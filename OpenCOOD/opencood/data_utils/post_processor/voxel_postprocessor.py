...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def generate_anchor_box(self) -> torch.Tensor:
        ...
        length = self.params["anchor_args"]["l"]
        w = self.params["anchor_args"]["w"]
        h = self.params["anchor_args"]["h"]
        ...
        if self.params["order"] == "hwl":
            anchors = np.stack([cx, cy, cz, h, w, length, r_], axis=-1)
        elif self.params["order"] == "lhw":
            anchors = np.stack([cx, cy, cz, length, h, w, r_], axis=-1)
        else:
