...
if __name__ == "__main__":
    ...
    opencda_dataset = EarlyFusionVisDataset(params, visualize=True, train=False)
    data_loader = DataLoader(
        opencda_dataset, batch_size=1, num_workers=8, collate_fn=opencda_dataset.collate_batch_train, shuffle=False, pin_memory=False
    )

    opt = vis_parser()
    vis_utils.visualize_sequence_dataloader(data_loader, params["postprocess"]["order"], color_mode=opt.color_mode)
