...
class RoIHead(nn.Module):
    ...
    def __init__(self, model_cfg: Dict[str, Any]) -> None:
        ...
        self.cls_layers, pre_channel = self._make_fc_layers(pre_channel, fc_layers, output_channels=self.model_cfg["num_cls"])
        self.iou_layers, _ = self._make_fc_layers(pre_channel, fc_layers, output_channels=self.model_cfg["num_cls"])
        self.reg_layers, _ = self._make_fc_layers(pre_channel, fc_layers, output_channels=self.model_cfg["num_cls"] * 7)
    ...
    def assign_targets(self, batch_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for rois, gts in zip(pred_boxes, gt_boxes):
            ...
            heading_label = (
                gt_of_rois[:, 6] + (torch.div(torch.abs(gt_of_rois[:, 6].min()), (2 * np.pi), rounding_mode="trunc") + 1) * 2 * np.pi
            ) % (2 * np.pi)  # 0 ~ 2pi
            ...
            reg_targets = box_utils.box_encode(gt_of_rois.view(-1, self.code_size), rois_anchor)
    ...
    def roi_grid_pool(self, batch_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # (BxN, 6x6x6, C)
        pooled_features = pooled_features.view(-1, self.grid_size**3, pooled_features.shape[-1])
    ...
    def forward(self, batch_dict):
        ...
        pooled_features = (
            pooled_features.permute(0, 2, 1).contiguous().view(batch_size_rcnn, -1, self.grid_size, self.grid_size, self.grid_size)
        )  # (BxN, C, 6, 6, 6)
