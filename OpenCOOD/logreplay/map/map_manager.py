...
from logreplay.map.map_utils import (
    ...
    list_wpt2array,
    convert_tl_status,
    exclude_off_road_agents,
    retrieve_city_object_info,
    obj_in_range,
)
from logreplay.map.map_drawing import cv2_subpixel, draw_agent, draw_road, draw_lane, road_exclude, draw_crosswalks, draw_city_objects
...
class MapManager(object):
    ...
    def __init__(self, world, config, output_root, scene_name):
        ...
        self.exclude_intersection_lane = config["static"]["exclude_intersection_lane"]
        ...
        # generate labels
        self.other_objs_info = retrieve_city_object_info(self.world, self.other_render_objs)
    ...
    def run_step(self, cav_id: str, cav_content: Dict[str, Any], veh_dict: Dict[str, Any]) -> None:
        ...
        if self.visibility:
            self.vis_ids = self.check_visibility_single(cav_content, self.vis_ids)
        if self.visibility_corp:
            self.vis_corp_ids = self.check_visibility_corp(veh_dict, self.vis_corp_ids)

        self.rasterize_static()
        self.rasterize_dynamic()

        if self.exclude_off_road:
            self.dynamic_bev = exclude_off_road_agents(self.static_bev, self.dynamic_bev)
    ...
    def get_bounds(left_lane: NDArray, right_lane: NDArray) -> NDArray:
        ...
        z_min = min(np.min(left_lane[:, 2]), np.min(right_lane[:, 2]))
        x_max = max(np.max(left_lane[:, 0]), np.max(right_lane[:, 0]))
        y_max = max(np.max(left_lane[:, 1]), np.max(right_lane[:, 1]))
        z_max = max(np.max(left_lane[:, 2]), np.max(right_lane[:, 2]))
    ...
    def data_dump(self) -> None:
        ...
        if self.save_yml:
            # save metadata
            save_yaml_name = os.path.join(save_name, self.current_timstamp + "_additional.yaml")
            save_yaml_wo_overwriting(map_info, save_yaml_name)

        if self.save_static:
            # save rgb image
            save_static_name = os.path.join(save_name, self.current_timstamp + "_bev_static.png")
            cv2.imwrite(save_static_name, self.static_bev)

        if self.save_lane:
            # save lane topology seg
            save_lane_name = os.path.join(save_name, self.current_timstamp + "_bev_lane.png")
            cv2.imwrite(save_lane_name, self.lane_bev)

        if self.save_dynamic:
            # save dynamic bev
            save_dynamic_name = os.path.join(save_name, self.current_timstamp + "_bev_dynamic.png")
            cv2.imwrite(save_dynamic_name, self.dynamic_bev)

        if self.visibility:
            # save  visibility map for ego agent
            save_visibility_name = os.path.join(save_name, self.current_timstamp + "_bev_visibility.png")
            cv2.imwrite(save_visibility_name, self.vis_mask)

        if self.visibility_corp:
            # save  visibility map for ego agent
            save_visibility_name = os.path.join(save_name, self.current_timstamp + "_bev_visibility_corp.png")
    ...
    def generate_lane_cross_info(self) -> None:
        ...
        for i, waypoint in enumerate(self.topology):
            ...
            intersection_flag = True if waypoint.is_intersection and self.exclude_intersection_lane else False
            # we need to exclude the intersections without traffic light/stops
            if intersection_flag:
                intersection_flag = False
                for t_pos in self.traffic_stop_pos:
                    distance = np.sqrt((t_pos[0] - waypoint.transform.location.x) ** 2 + (t_pos[1] - waypoint.transform.location.y) ** 2)
                    if distance <= 30:
    ...
    def generate_cross_area(self, xyz: NDArray[np.float64]) -> NDArray[np.float64]:
        ...
        # convert coordinates to center's coordinate frame
        xyz = xyz.T
        xyz = np.r_[xyz, [np.ones(xyz.shape[1])]]
    ...
    def rasterize_static(self) -> None:
        ...
        cross_indices = self.indices_in_bounds(self.bound_info["crosswalks"]["bounds"], raster_radius)
        ...
        if self.draw_lane:
            self.lane_bev = draw_lane(lanes_area_list, lane_type_list, self.lane_bev, intersection_list, vis=False)
