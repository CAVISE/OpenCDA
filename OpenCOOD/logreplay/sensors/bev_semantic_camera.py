...
class BEVSemanticCamera(BaseSensor):
    ...
    def spawn_point_estimation(self, global_position: Optional[List[float]]) -> carla.Transform:
        ...
        if global_position is not None:
            carla_location = carla.Location(x=global_position[0], y=global_position[1], z=self.height)
    ...
    def data_dump(self, output_root: str, cur_timestamp: str) -> None:
        ...
        while not hasattr(self, "vis_image") or self.vis_image is None:
            continue
        # dump visualization
        output_vis_name = os.path.join(output_root, cur_timestamp + "_bev_sem_vis.png")
        ...
        # dump the yaml
        save_yaml_name = os.path.join(output_root, cur_timestamp + "_additional.yaml")
        # intrinsic
        ...
        bev_sem_cam_info = {"bev_sem_cam": {"intrinsic": camera_intrinsic, "cords": cords}}
