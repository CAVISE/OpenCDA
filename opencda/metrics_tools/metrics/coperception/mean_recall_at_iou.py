...
logger = logging.getLogger("cavise.opencda.opencda.metrics_tools.metrics.coperception.mean_recall_at_iou")
...
class MeanRecallAtIoUMetric(BaseMetric):
    ...
    def _process_context(self, context: Mapping[str, Any]) -> None:
        ...
        for iou in self.iou_thresholds:
            self._samples_by_series[self._series_name(iou)].append(self._make_sample(self.calculate_mean_recall(iou)))
    ...
    def calculate_mean_recall(self, iou: float) -> float:
        return mean_curve_value(self._calculate_mrec(iou))
