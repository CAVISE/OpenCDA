...
class AdvCoperceptionIntermediateFusionAttack:
    ...
    def _model_boxes_to_lwh(boxes: torch.Tensor, dataset: Any) -> torch.Tensor:
        ...
        ``"hwl"`` axis ordering for the box dimensions. The IoU
        computation expects ``"lwh"``; this helper rearranges the
        columns when needed and returns the original tensor when the
    ...
    def _compute_iou_weights(proposals_lwh: torch.Tensor, target_box_lwh: torch.Tensor) -> torch.Tensor:
        """
        Compute 2D IoU between every proposal and a single target box.

        Both the proposals and the target are projected to their 2D
        oriented bounding boxes (XY corners only); the intersection
        area is computed analytically by
        :meth:`_oriented_box_intersection_2d` and the union derived
        from individual areas.

        Returns
        -------
        torch.Tensor
            ``(num_proposals,)`` IoU values in ``[0, 1]``.
        """
        if proposals_lwh.shape[0] == 0:
            return torch.zeros((0,), dtype=target_box_lwh.dtype, device=target_box_lwh.device)

        # Broadcast the target across all proposals; both tensors then
        # share the leading "proposal" dimension and can be processed
        # in one batched call.
        ...
        # 2D area = length * width (the L and W columns of the
        # ordered box, indices 3 and 4).
    ...
    def _oriented_box_intersection_2d(
        ...
        Implements the standard polygon-clipping recipe:

        1. The intersection polygon is the convex hull of three vertex
           sets:

           - corners of box 1 lying inside box 2,
           - corners of box 2 lying inside box 1,
           - edge-edge intersection points between the two boxes.

        2. Sort the vertices of that convex set by angle around their
           centroid and compute the polygon area via the shoelace
           formula.

        Both arguments must be batches of 4-corner XY arrays (shape
        ``(B, 4, 2)``); the result is a length-``B`` tensor of
        intersection areas. Differentiable everywhere except at the
        zero-area boundary, which is good enough for gradient-based
        attacks.
        """
        ...
        # Concatenate up to 24 candidate vertices per box pair: 4
        # corners1 + 4 corners2 + up to 16 edge-edge intersections.
        # The boolean mask retains only the ones that actually belong
    ...
    def _box_intersection_th(
        ...
        """
        Compute every edge-edge intersection between two batched
        quadrilaterals.

        For each box pair, builds the four directed edges
        ``(corners[i], corners[i+1])``, then computes the line-line
        intersection between every pair of edges (4 x 4 = 16 per pair).
        The mask flags valid intersections (parameters ``t`` and ``u``
        both in ``(0, 1)``); invalid entries are zeroed in the
        intersection tensor so downstream code can ignore them via the
        mask.

        Returns
        -------
        tuple
            ``(intersections, mask)`` of shapes
            ``(B, 4, 4, 2)`` and ``(B, 4, 4)``.
        """
        epsilon = 1e-8
    ...
    def _box_in_box_th(
        ...
        Returns a pair of boolean tensors: which of ``corners1`` lie
        inside ``corners2`` and vice versa.
        """
    ...
    def _box1_in_box2(corners1: torch.Tensor, corners2: torch.Tensor) -> torch.Tensor:
        ...
        Treats box 2 as a parallelogram defined by its corner ``A``
        and the edge vectors ``AB`` and ``AD``. A point ``M`` lies
        inside iff its parametric coordinates along both edges are in
        ``[0, 1]``:

        ``s = (AM . AB) / (AB . AB), t = (AM . AD) / (AD . AD)``

        The check uses a small slack (``+/-1e-6``) to keep boundary
        points inside.
        """
    ...
    def _calculate_polygon_area(vertices: torch.Tensor, vertex_mask: torch.Tensor) -> torch.Tensor:
        ...
        1. Compute the centroid of valid vertices (their mean).
        2. Sort all vertices by their angle around that centroid;
           invalid vertices are pushed to the end via a ``+inf``
           sentinel angle so they end up unused.
        3. Apply the shoelace formula on the first ``num_valid``
           vertices.

        Polygons with fewer than three valid vertices have area zero
        (no actual intersection).

        Returns
        -------
        torch.Tensor
            ``(B,)`` polygon areas. Differentiable in vertex
            coordinates.
        """
