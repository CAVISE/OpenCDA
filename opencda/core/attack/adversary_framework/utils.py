...
def get_capability_binding(
    ...
    try:
        ...
        raise RuntimeError(
            f"Service '{service.service_name}' does not expose capability '{capability.value}'."
        ) from exc
...
def install_output_interceptor(
    ...
    *,
    ...
    if not method_name:
        raise RuntimeError(
            f"Could not resolve method name for capability '{capability.value}' "
            f"on service '{service.service_name}'."
        )
