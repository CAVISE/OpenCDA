...
from opencda.core.common.misc import distance_vehicle, draw_trajetory_points, cal_distance_angle, compute_distance
...
class LocalPlanner(object):
    ...
    def generate_path(self) -> Tuple[List[float], List[float], List[float], List[float]]:
        ...
        vec_norm, angle = cal_distance_angle(previous_wpt.transform.location, future_wpt.transform.location, future_wpt.transform.rotation.yaw)
    ...
    def generate_trajectory(self, rx: List[float], ry: List[float], rk: List[float]) -> None:
        ...
        for i in range(1, int(sample_num) + 1):
            ...
            self._trajectory_buffer.append(
                (carla.Transform(carla.Location(sample_x, sample_y, self._waypoint_buffer[0][0].transform.location.z + 0.5)), target_speed)
            )
            if break_flag:
                break
    ...
    def buffer_filter(self) -> None:
        ...
        for i, (waypoint, _) in enumerate(tmp):
            ...
            if prev_wpt.lane_id != waypoint.lane_id and len(self._waypoint_buffer) >= 2:
                dist = compute_distance(waypoint.transform.location, prev_wpt.transform.location)
    ...
    def pop_buffer(self, vehicle_transform: carla.Position) -> None:
        ...
        if max_index >= 0:
            for i in range(max_index + 1):
                if self._history_buffer:
                    ...
                    if (
                        abs(prev_wpt[0].transform.location.x - incoming_wpt[0].transform.location.x) > 4.5
                        or abs(prev_wpt[0].transform.location.y - incoming_wpt[0].transform.location.y) > 4.5
                    ):
        ...
        if self._trajectory_buffer:
            ...
            for i, (
                waypoint,
                _,
            ) in enumerate(self._trajectory_buffer):
                if distance_vehicle(waypoint, vehicle_transform) < max(self._min_distance - 1, 1):
                    max_index = i
            if max_index >= 0:
                for i in range(max_index + 1):
    ...
    following: bool = False
        ...
        if self.debug_trajectory:
            draw_trajetory_points(self._vehicle.get_world(), self._long_plan_debug, color=carla.Color(0, 255, 0), size=0.05, lt=0.1)
