...
from .util import read_pcd, bbox_map_to_sensor, bbox_sensor_to_map
    ...
    def _build_cases(self) -> None:
        ...
        for scenario_id, scenario_data in self.meta.items():
            for frame_id, frame_data in scenario_data["data"].items():
                self.cases["multi_vehicle"].append({"scenario_id": scenario_id, "frame_id": frame_id})
                frame_ids.append(frame_id)
                if len(frame_ids) == frame_num_per_case:
                    self.cases["multi_frame"].append({"scenario_id": scenario_id, "frame_ids": frame_ids})
                    frame_ids = []
    ...
    def get_case_by_meta(
        ...
        if tag == "single_vehicle":
            ...
            gt_bboxes_9d[:, [6, 8]] = np.array(
                [[object_data["angle"][0], object_data["angle"][2]] for object_id, object_data in calib["vehicles"].items()]
            )
