...
def ensure_comm_protos_ready():
    ...
    Generated modules may use top-level imports (e.g. `import entity_pb2`), so we temporarily add
    the output directory to sys.path for imports. The modules are generated by the CMake build step.
    """
    import pathlib
    ...
    try:
        try:
            ...
            # Fast-path import/validation failed; report the missing CMake-generated artifacts.
            pass

        raise AssertionError(
            "Generated communication protobuf modules are missing or stale. "
            "Run CMake with OPENCDA_BUILD_PROTOBUF=ON and install the build output before running these tests."
        )
    finally:
...
def fake_env(monkeypatch, ensure_comm_protos_ready):
    ...
    - Ensures protobuf Python modules are available.
