...
if __name__ == "__main__":
    ...
    with open("OpenCDA_message.proto", 'rb') as f:
        previous_output_msg = f.read()
        opencda_msg = OpenCDA_message()
        opencda_msg.ParseFromString(previous_output_msg)
        if len(opencda_msg.cav) == 3:
            print("отправили")
            cm1.send_message(previous_output_msg)
    ...
    while True:
        with open("OpenCDA_message.proto", 'rb') as f:
            ...
            if len(opencda_msg.cav) == 3 and output_msg != previous_output_msg:
                print("отправили")
