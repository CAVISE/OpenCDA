...
import os
import yaml
import opencda.core.common.OpenCDA_message_structure_pb2 as pb2
...
if __name__ == "__main__":
    ...
    while True:
        with open("OpenCDA_message.proto", 'rb') as f:
            output_msg = f.read()

        opencda_msg = OpenCDA_message()
        opencda_msg.ParseFromString(output_msg)
        if len(opencda_msg.cav) == 3:

            cm1.send_message(output_msg)
            previous_output_msg = output_msg

            

            input_msg = cm1.receive_message()
            artery_msg = Artery_message()
            artery_msg.ParseFromString(input_msg)
            # Сохранение в формате JSON
            json_output = MessageToJson(artery_msg, including_default_value_fields=True, preserving_proto_field_name=True)
            with open("Messages/Artery_message.json", "w") as json_file:
                json_file.write(json_output)

            # Сохранение в формате бинарника .proto
            with open("Messages/Artery_message.proto", "wb") as proto_file:
                proto_file.write(input_msg)
