...
from mvp.data.util import bbox_map_to_sensor, bbox_sensor_to_map
    ...
    def run(self, multi_frame_case: Dict[int, Any], attack_opts: Dict[str, Any]) -> Tuple[Dict[int, Any], List[Dict[int, Any]]]:
        ...
        for frame_index, frame_id in enumerate(attack_opts["frame_ids"]):
            ...
            bbox_to_remove_ego = bbox_map_to_sensor(
                bbox_sensor_to_map(bbox_to_remove, optimize_case[attacker_id]["lidar_pose"]), optimize_case[ego_id]["lidar_pose"]
            )
            try:
            ...
            real_bbox_to_remove_ego = bbox_map_to_sensor(
                bbox_sensor_to_map(real_bbox_to_remove, case[frame_id][attacker_id]["lidar_pose"]), case[frame_id][ego_id]["lidar_pose"]
            )

            if self.init:
                optimize_case[attacker_id]["lidar"] = self.apply_ray_tracing(
                    optimize_case[attacker_id]["lidar"], **attack_opts["attack_info"][frame_id if self.sync == 0 else frame_id - 1]
                )
