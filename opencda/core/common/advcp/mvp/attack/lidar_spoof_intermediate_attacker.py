...
class LidarSpoofIntermediateAttacker(Attacker):
    def __init__(
        ...
        if step <= 2:
            self.learn_rate: Union[int, float] = 1
        else:
    ...
    def run(self, multi_frame_case: Dict[int, Any], attack_opts: Dict[str, Any]) -> Tuple[Dict[int, Any], List[Dict[int, Any]]]:
        ...
        for frame_index, frame_id in enumerate(attack_opts["frame_ids"]):
            ...
            bbox_to_spoof_ego = bbox_map_to_sensor(
                bbox_sensor_to_map(bbox_to_spoof, optimize_case[attacker_id]["lidar_pose"]), optimize_case[ego_id]["lidar_pose"]
            )
            real_bbox_to_spoof = attack_opts["positions"][frame_id]
            real_bbox_to_spoof_ego = bbox_map_to_sensor(
                bbox_sensor_to_map(real_bbox_to_spoof, case[frame_id][attacker_id]["lidar_pose"]), case[frame_id][ego_id]["lidar_pose"]
            )
