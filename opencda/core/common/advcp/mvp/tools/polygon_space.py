...
def get_free_space_fast(
    ...
    is_object = np.logical_or(
        in_lane_mask == 0,
        np.logical_or(np.logical_and(point_height > height_thres, point_height < 3), np.logical_and(object_mask, np.logical_not(height_mask))),
    )
...
def get_free_space(
    ...
    pcd[height_mask, :3] -= np.tile(point_height[height_mask] - height_thres, (3, 1)).T * rays[height_mask]
    is_object = np.logical_or(
        in_lane_mask == 0, np.logical_or(point_height > height_thres + height_tolerance, np.logical_and(object_mask, np.logical_not(height_mask)))
    )
    ...
    for angle in range(angle_split):
        ...
        for ring in range(ray_count):
            ...
            if free_space_map[angle, ring] > 0 and (
                len(further_object_indices) == 0 or distance_map[angle, further_object_indices + ring, 0].min() > distance_map[angle, ring, 0]
            ):
                if ring_start is None:
                    ring_start = ring
                elif ring == ray_count - 1 and ring_start < ray_count - 1:
                    polygons.append(
                        sensor_points_to_map_polygon(
                            [
