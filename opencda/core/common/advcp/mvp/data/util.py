...
def bbox_map_to_sensor(bbox, sensor_calib):
    ...
    if bbox.ndim == 1:
        ...
        new_bbox[:3] = np.dot(np.linalg.inv(rotation_matrix(*(sensor_rotation))), new_bbox[:3].T).T
...
def bbox_sensor_to_map(bbox, sensor_calib):
    ...
    if bbox.ndim == 1:
        new_bbox[6] += sensor_rotation[1]
        new_bbox[:3] = np.dot(rotation_matrix(*(sensor_rotation)), new_bbox[:3].T).T
        new_bbox[:3] += sensor_location
    elif bbox.ndim == 2:
        new_bbox[:, 6] += sensor_rotation[1]
        new_bbox[:, :3] = np.dot(rotation_matrix(*(sensor_rotation)), new_bbox[:, :3].T).T
...
def pcd_sensor_to_map(pcd, sensor_calib):
    ...
    sensor_rotation = sensor_calib[3:] * np.pi / 180
    new_pcd = np.copy(pcd)
    new_pcd[:, :3] = np.dot(rotation_matrix(*(sensor_rotation)), new_pcd[:, :3].T).T
...
def pcd_map_to_sensor(pcd, sensor_calib):
    ...
    new_pcd = np.copy(pcd)
    new_pcd[:, :3] -= sensor_location
    new_pcd[:, :3] = np.dot(np.linalg.inv(rotation_matrix(*(sensor_rotation))), new_pcd[:, :3].T).T
    return new_pcd
...
def corners_to_bbox_batch(corners):
    ...
    h = abs(np.mean(corners[:, 4:, 2] - corners[:, :4, 2], axis=1, keepdims=True))
    length = (
        np.sqrt(np.sum((corners[:, 0, [0, 1]] - corners[:, 3, [0, 1]]) ** 2, axis=1, keepdims=True))
        + np.sqrt(np.sum((corners[:, 2, [0, 1]] - corners[:, 1, [0, 1]]) ** 2, axis=1, keepdims=True))
        + np.sqrt(np.sum((corners[:, 4, [0, 1]] - corners[:, 7, [0, 1]]) ** 2, axis=1, keepdims=True))
        + np.sqrt(np.sum((corners[:, 5, [0, 1]] - corners[:, 6, [0, 1]]) ** 2, axis=1, keepdims=True))
    ) / 4

    w = (
        np.sqrt(np.sum((corners[:, 0, [0, 1]] - corners[:, 1, [0, 1]]) ** 2, axis=1, keepdims=True))
        + np.sqrt(np.sum((corners[:, 2, [0, 1]] - corners[:, 3, [0, 1]]) ** 2, axis=1, keepdims=True))
        + np.sqrt(np.sum((corners[:, 4, [0, 1]] - corners[:, 5, [0, 1]]) ** 2, axis=1, keepdims=True))
        + np.sqrt(np.sum((corners[:, 6, [0, 1]] - corners[:, 7, [0, 1]]) ** 2, axis=1, keepdims=True))
    ) / 4

    theta = (
        np.arctan2(corners[:, 1, 1] - corners[:, 2, 1], corners[:, 1, 0] - corners[:, 2, 0])
        + np.arctan2(corners[:, 0, 1] - corners[:, 3, 1], corners[:, 0, 0] - corners[:, 3, 0])
        + np.arctan2(corners[:, 5, 1] - corners[:, 6, 1], corners[:, 5, 0] - corners[:, 6, 0])
        + np.arctan2(corners[:, 4, 1] - corners[:, 7, 1], corners[:, 4, 0] - corners[:, 7, 0])
    )[:, np.newaxis] / 4

    bbox = np.concatenate([xyz, length, w, h, theta], axis=1).reshape(batch_size, 7)
    bbox[:, 2] -= 0.5 * bbox[:, 5]
    return bbox
