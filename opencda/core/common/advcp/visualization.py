...
class Visualization:
    ...
    def visualize_results(self, pred_box_tensor: Any, gt_box_tensor: Any, batch_data: Dict, tick_number: int):
        ...
        self.history.append({
            "tick": tick_number,
            "geometries": self.vis.get_geometries(),
        ...
        for cav_id, cav_data in batch_data.items():
            ...
            if trust_score < 0.7:
                ...
                self._add_text_geometry(
                    f"CAV {cav_id}: Trust {trust_score:.2f}",
                    position=cav_data.get("advcp_metadata", {}).get("position", [0, 0, 0]),
                    color=[1, 0, 0]
                )
            ...
            if cav_data.get("advcp_metadata", {}).get("blocked_attacks", 0) > 0:
                self._add_text_geometry(
                    f"Blocked {cav_data.get(\"advcp_metadata\", {}).get(\"blocked_attacks\", 0)} attacks",
                    position=cav_data.get("advcp_metadata", {}).get("position", [0, 0, 0]),
                    color=[0, 1, 0]
                )
