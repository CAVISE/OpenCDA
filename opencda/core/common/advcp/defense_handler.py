...
class DefenseHandler:
    ...
    def apply_defense(self, batch_data: Dict, tick_number: int) -> Dict:
        ...
        # Check consistency
        consistency_results = self.consistency_checker.check_consistency(batch_data, tick_number)

        # Detect anomalies
        anomaly_results = self.anomaly_detector.detect_anomalies(batch_data, consistency_results, tick_number)

        # Update trust scores
        self._update_trust_scores(batch_data, anomaly_results, tick_number)

        # Filter malicious data
        modified_data = self._filter_malicious_data(modified_data, anomaly_results, tick_number)
    ...
    def _update_trust_scores(self, batch_data: Dict, anomaly_results: Dict, tick_number: int):
        ...
        for cav_id, cav_data in batch_data.items():
            ...
            # Store in history
            self.history.append({"tick": tick_number, "cav_id": cav_id, "trust_score": trust_score, "anomaly_score": anomaly_score})
...
class ConsistencyChecker:
    ...
    def check_consistency(self, batch_data: Dict, tick_number: int) -> Dict:
        ...
        for i, cav_id in enumerate(cav_ids):
            ...
            results[cav_id] = {"consistency_score": avg_consistency, "consistent": avg_consistency > self.min_consistency}

        # Store in history
        self.history.append({"tick": tick_number, "results": results.copy()})
...
class AnomalyDetector:
    ...
    def detect_anomalies(self, batch_data: Dict, consistency_results: Dict, tick_number: int) -> Dict:
        ...
        for cav_id, cav_data in batch_data.items():
            ...
            results[cav_id] = {"anomaly_score": anomaly_score, "is_anomaly": is_anomaly, "consistency": consistency}
