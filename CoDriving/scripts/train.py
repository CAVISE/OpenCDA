...
from data_config import EXPIREMENTS_PATH, DATA_PATH, EXPIREMENTS_MODELS_CONFIG_PATH, EXPIREMENTS_TRAIN_CONFIG_PATH
...
def train_many_configs(max_needed_gpu_usage=0.7, max_mem_usage=0.7, max_processes=7, wait_time=4):
    ...
    while task_queue or len(active_processes):
        while (
            ...
            p = mp.Process(target=train_one_config, args=(config_pair[0], config_pair[1], EXPIREMENTS_PATH, DATA_PATH, device_str))
            p.start()
