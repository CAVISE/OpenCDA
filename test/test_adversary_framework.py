...
class TestConditionEvaluator:
    ...
    def test_a_cond_01_verb_eq_true(self):
        ...
        snapshot = _make_snapshot(vehicle_nodes=(_make_node(service_states={"svc": {"f": 10}}),))
    ...
    def test_a_cond_03_verb_gte(self):
        ...
        snap_true = _make_snapshot(vehicle_nodes=(_make_node(service_states={"svc": {"f": 5}}),))
        ...
        snap_false = _make_snapshot(vehicle_nodes=(_make_node(service_states={"svc": {"f": 4}}),))
    ...
    def test_a_cond_04_verbs_lt_lte(self):
        ...
        snap = _make_snapshot(vehicle_nodes=(_make_node(service_states={"svc": {"f": 5}}),))
    ...
    def test_a_cond_05_verb_exists(self):
        ...
        snap_present = _make_snapshot(vehicle_nodes=(_make_node(service_states={"svc": {"f": 1}}),))
        assert evaluate_condition(runtime, cond_exists, None, snap_present) is True

        snap_absent = _make_snapshot(vehicle_nodes=(_make_node(service_states={"svc": {}}),))
    ...
    def test_a_cond_06_verb_changed(self):
        ...
        prev = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": 1}}),))
        curr = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": 2}}),))
        assert evaluate_condition(runtime, condition, prev, curr) is True

        curr_same = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": 1}}),))
    ...
    def test_a_cond_07_verb_became(self):
        ...
        prev = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": 1}}),))
        curr = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": 42}}),))
        assert evaluate_condition(runtime, condition, prev, curr) is True

        curr_wrong = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": 1}}),))
    ...
    def test_a_cond_08_verb_increased_no_delta(self):
        ...
        prev = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": 5}}),))
        curr = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": 10}}),))
        assert evaluate_condition(runtime, condition, prev, curr) is True
    ...
    def test_a_cond_09_verb_decreased_no_delta(self):
        ...
        prev = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": 10}}),))
        curr = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": 5}}),))
        assert evaluate_condition(runtime, condition, prev, curr) is True
    ...
    def test_a_cond_10_verb_added(self):
        ...
        prev = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": ["item_a"]}}),))
        curr = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": ["item_a", "item_b"]}}),))
        assert evaluate_condition(runtime, condition, prev, curr) is True
    ...
    def test_a_cond_11_verb_removed(self):
        ...
        prev = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": ["item_a", "item_b"]}}),))
        curr = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": ["item_a"]}}),))
        assert evaluate_condition(runtime, condition, prev, curr) is True
    ...
    def test_a_cond_12_added_removed_none_value(self):
        ...
        prev = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": ["a"]}}),))
        curr = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": ["a", "b"]}}),))
    ...
    def test_a_cond_13_verb_increased_with_delta(self):
        ...
        prev = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": 0}}),))
        curr = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": 10}}),))
        assert evaluate_condition(runtime, condition, prev, curr) is True

        curr_small = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": 3}}),))
    ...
    def test_a_cond_14_verb_decreased_with_delta(self):
        ...
        prev = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": 10}}),))
        curr = _make_snapshot(vehicle_nodes=(_make_node(node_id="v1", service_states={"svc": {"f": 0}}),))
        assert evaluate_condition(runtime, condition, prev, curr) is True
    ...
    def test_a_cond_18_any_all_false(self):
        ...
        condition = ConditionSpec(
            any=(
                _make_leaf_condition(verb="gt", value=100, field="f"),
                ConditionSpec(all=(_make_leaf_condition(verb="gt", value=200, field="f"),)),
            ),
        )
        snapshot = _make_snapshot(vehicle_nodes=(_make_node(service_states={"svc": {"f": 10}}),))
        assert evaluate_condition(runtime, condition, None, snapshot) is False
    ...
    def test_a_cond_19_nested_all_any_deep_recursion(self):
        ...
        condition = ConditionSpec(
            any=(
                ConditionSpec(
                    all=(_make_leaf_condition(verb="gt", value=0, field="f"),),
                ),
    ...
    def test_a_cond_26_unsupported_source_kind_raises(self):
        ...
        snapshot = _make_snapshot(vehicle_nodes=(_make_node(service_states={"svc": {"f": 1}}),))
...
class TestAttack:
    ...
    def test_a_atk_09_should_stop_active_with_trigger(self):
        ...
        attack.mark_active()
        snapshot = _make_snapshot(vehicle_nodes=(_make_node(service_states={"svc": {"f": 0}}),))
    ...
    def test_a_atk_13_stage_not_launched_trigger_unmet(self):
        ...
        mock_stage_0.execute.return_value = AttackStageResult(stage_name="s0", status=Status.SUCCESS)
    ...
    def test_a_atk_15_propagation_fail_from_stage(self):
        ...
        mock_stage_0.stage_name = "s0"
        mock_stage_0.required_capabilities = ()
        mock_stage_0.execute.return_value = AttackStageResult(stage_name="s0", status=Status.FAIL, reason="failed")
    ...
    def test_a_atk_16_propagation_stop_from_stage(self):
        ...
        mock_stage.stage_name = "s0"
        mock_stage.required_capabilities = ()
        mock_stage.execute.return_value = AttackStageResult(stage_name="s0", status=Status.STOP, reason="stopped")
    ...
    def test_a_atk_17_all_stages_success_attack_succeeded(self):
        ...
        mock_s0.execute.return_value = AttackStageResult(stage_name="s0", status=Status.SUCCESS)

        mock_s1 = MagicMock()
        mock_s1.stage_name = "s1"
        mock_s1.required_capabilities = ()
        mock_s1.execute.return_value = AttackStageResult(stage_name="s1", status=Status.SUCCESS)
    ...
    def test_a_atk_18_stop_trigger_deactivates_stage(self):
        ...
        snapshot_start = _make_snapshot(vehicle_nodes=(_make_node(service_states={"svc": {"f": 1}}),))
        dummy_service = _make_mock_service()
        attack.mark_started()
        attack.mark_active()
...
class TestAttackManager:
    ...
    def test_a_mgr_05_no_restart_after_stop(self):
        ...
        snap1 = _make_snapshot(vehicle_nodes=(_make_node(service_states={"svc": {"f": 1}}),))
    ...
    def test_a_mgr_06_active_attack_run_stage_lifecycle_called(self):
        ...
        def service_resolver(node_id, svc_type):
            return mock_service
...
class TestUtils:
    ...
    def test_a_util_01_install_output_interceptor_patches(self):
        ...
        service = _make_mock_service(capability_bindings={Capability.RESPONSE_OBSERVE: lambda: "original"})
        ...
        restore = install_output_interceptor(service, Capability.RESPONSE_OBSERVE, rewrite_result=rewrite)
    ...
    def test_a_util_03_idempotent_restore(self):
        ...
        restore = install_output_interceptor(service, Capability.RESPONSE_OBSERVE, rewrite_result=lambda r: r)
        restore()
    ...
    def test_a_util_06_match_services(self):
        ...
        svc_partial = _make_mock_service(capability_bindings={Capability.RESPONSE_OBSERVE: lambda: None})
...
class TestResponseReplayerStage:
    ...
    def test_a_rpl_05_third_call_replays_second(self):
        ...
        service.response_submit()  # pass-through (no previous)
    ...
    def test_a_rpl_07_separate_history_per_service(self):
        ...
        service_a.response_submit()  # a_1 (pass-through)
        service_b.response_submit()  # b_1 (pass-through) # b_1 (pass-through)
